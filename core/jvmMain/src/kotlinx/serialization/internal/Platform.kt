...
internal actual fun KClass<*>.platformSpecificSerializerNotRegistered(): Nothing {
    // Check if the class is annotated with @Serializable with the default 'with' parameter.
    // This indicates the serialization compiler plugin was supposed to generate a serializer
    // but was not applied (e.g., in Kotlin script files where @file:CompilerOptions("-Xplugin=...")
    // is silently ignored — see https://youtrack.jetbrains.com/issue/KT-47384).
    val serializableAnnotation = try {
        java.getAnnotation(Serializable::class.java)
    } catch (_: Throwable) {
        null
    }
    if (serializableAnnotation != null && serializableAnnotation.with.java == KSerializer::class.java) {
        throw SerializationException(
            notRegisteredMessage() +
                    "The class is annotated with '@Serializable' but no serializer was generated by the compiler plugin.\n" +
                    "Ensure that the serialization compiler plugin is applied to the module where the class is defined.\n" +
                    "If you are using Kotlin scripts (.main.kts), note that the compiler plugin cannot be applied\n" +
                    "via @file:CompilerOptions(\"-Xplugin=...\") — that option is ignored by the script compiler.\n" +
                    "Use a Gradle project (applying 'org.jetbrains.kotlin.plugin.serialization') instead.\n"
        )
    }
